...
import DessertsCard from "../../components/CategoriesPagesCard/DessertsCard";
...
const Desserts = () => {
  const { data, isLoading, error } = useQuery({
    queryKey: ["desserts"],
    queryFn: async () => {
      const response = await fetch("https://foodpoint-backend-venk.onrender.com/desserts");
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5 p-5">
        {data.map((desserts) => (
          <Fade
            ...
            key={desserts._id}
          >
            <DessertsCard desserts={desserts} />
          </Fade>
        ))}
      </div>
    </div>
