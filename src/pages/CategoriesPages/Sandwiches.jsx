...
import SandwichesCard from "../../components/CategoriesPagesCard/SandwichesCard";
...
const Sandwiches = () => {
  const { data, isLoading, error } = useQuery({
    queryKey: ["sandwiches"],
    queryFn: async () => {
      const response = await fetch("https://foodpoint-backend-venk.onrender.com/sandwiches");
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5 p-5">
        {data.map((sandwiches) => (
          <Fade
            ...
            key={sandwiches._id}
          >
            <SandwichesCard sandwiches={sandwiches} />
          </Fade>
        ))}
      </div>
    </div>
