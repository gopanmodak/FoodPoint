...
import FrideChickenCard from "../../components/CategoriesPagesCard/FrideChickenCard";


const FrideChicken = () => {
  const { data, isLoading, error } = useQuery({
    queryKey: ["fride-chicken"],
    queryFn: async () => {
      const response = await fetch("https://foodpoint-backend-venk.onrender.com/friedchicken");
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5 p-5">
        {data.map((friedchicken) => (
          <Fade
            cascade
            damping={0.5}
            direction="up"
            triggerOnce
            key={friedchicken._id}
          >
            <FrideChickenCard friedchicken={friedchicken} />
          </Fade>
        ))}
      </div>
    </div>
